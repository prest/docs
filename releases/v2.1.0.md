...
**v2.1.0** adds native [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) support over HTTP ([#977](https://github.com/prest/prest/pull/977)), closing [#959](https://github.com/prest/prest/issues/959). MCP clients can discover and query your Postgres catalog through the same pREST server that already serves CRUD, custom scripts, and multi-database routes.
...
See the [MCP over HTTP guide](../get-started/mcp-over-http.md) for full usage, tool reference, and examples. For stdio clients (Cursor, Claude Desktop), install the [pREST MCP Adapter](../get-started/prest-mcp-adapter.md) via [`brew install prest/tap/prest-mcp`](../get-prest/start-with-homebrew.md).
