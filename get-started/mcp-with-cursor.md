...
Use [Cursor](https://cursor.com/) with pREST’s read-only MCP tools so the agent can discover schemas and query Postgres through your running pREST instance.

pREST speaks MCP over HTTP at `/_mcp`. Cursor launches **stdio** MCP servers, so you need the official [prest-mcp](prest-mcp-adapter.md) adapter as a bridge.
...
Other install options (Go, Docker): [pREST MCP Adapter](prest-mcp-adapter.md#install). Confirm the binary is on your `PATH`:
...
The agent should call tools such as `prest.list_databases`, `prest.describe_table`, or `prest.select.{database}.{schema}.{table}`. Tool behavior and limits are documented in [MCP over HTTP](mcp-over-http.md).
